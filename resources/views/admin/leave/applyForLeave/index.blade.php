									...
									<tbody>
										...
										@foreach($results AS $value)
											<tr>
												...
												<td style="width: 100px;">@if(isset($value->head_remarks)) {!! $value->head_remarks !!} @endif</td>
												<td>
													@if(isset($value->approve_date))
														{!! !is_null($value->approve_by) ? $value->approveBy->first_name : '' !!} {!! !is_null($value->approve_by) ? $value->approveBy->last_name : '' !!}
													@if(isset($value->approveBy))	
														<br/><span class="text-muted">@lang('leave.approve_date') : {!! dateConvertDBtoForm($value->approve_date) !!}</span>
													@endif
													@endif
												</td>
												<td>
													@if(isset($value->rejectBy))
														{!! !is_null($value->reject_by) ? $value->rejectBy->first_name : '' !!} {!! !is_null($value->reject_by) ? $value->rejectBy->last_name : '' !!}
													@if(isset($value->reject_date))
